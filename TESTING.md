...
Thanks for trying Run Hound. The repository is public and open source: anyone can try it (no clone needed with Docker), read the code and file issues. Run Hound is AI-assisted UI testing for AI-built apps: AI plans and explains, real checks decide. This preview ships the real checks; 0.4.0 adds signed-in runs with two test accounts and the first access checks, and handles the widgets and dialogs AI app builders generate; 0.5.0 adds the write-side checks (can another account, a signed-out visitor or another website change your data, and can a paid plan be had without paying). Since 0.3.0 you can add your own model to review the plan, suggest extra flows and explain findings (optional, off by default). This guide covers what Run Hound does, how to run it on Kennel (the demo app) and then on your own app, signed-in runs, how to read the report, and what to send back.
...
- **Write-side checks** (signed in, unticked by default): `write-access` (can account B, or a visitor who isn't signed in, change or delete account A's test record?), `csrf` (can a page on another site make A's browser change A's data?) and `paywall-trust` (can A get a paid plan without paying?). They write only the test record Run Hound just created as A, judge from a re-read as A, and put back what they changed. See [The write-side checks](#the-write-side-checks-050).
...
| `write-access` (0.5.0, signed in) | Saves a test record as A, then sends the update and delete requests the app itself used for it, as account B and with no session, and re-reads it as A after each. A change that shows is a finding. Restores the record. Unticked by default. | 1 per scenario, in account A |
...
| `paywall-trust` (0.5.0, signed in) | Finds A's plan or entitlement data, opens the app's own upgrade success pages and replays its own upgrade request with a zero price or a paid plan, then re-reads the plan as A. A paid plan without paying is a finding. Never calls a payment provider; restores the plan. Unticked by default. | 0 |

The checks are grouped as **Accessibility** (`axe-states`, `keyboard-completion`, `focus-visible`, `error-announcement`, `credential-fields`, `reflow-320`), **Features** (`console-network-errors`, `dead-control`, `silent-failure`, `persistence`, `double-submit`, `client-only-validation`, `page-controls`, `ai-flow`, `deep-links`) and **Security** (`bundle-secrets`, `pii-leak`, `verbose-errors`, `security-headers`, `cookie-flags`, `cors`, `source-maps`, `access-control`, `mass-assignment`, `write-access`, `csrf`, `paywall-trust`). The plan, the run, the progress and the report all follow that order.

Form checks (and `mass-assignment`, `write-access` and `csrf`) run once per form; on a page with several forms their scenarios are named after the form ("… (Newsletter form)"). `page-controls`, `security-headers`, `cookie-flags`, `cors`, `source-maps`, `bundle-secrets`, `focus-visible`, `reflow-320`, `access-control`, `deep-links` and `paywall-trust` run once for the whole page. A search form gets only the checks that make sense for it (it saves nothing), and a form that never shows what it saved (a newsletter signup) has its `persistence` scenario skipped with that reason rather than reported as lost data. Buttons that sign you out, cancel a subscription or empty a cart are never clicked unless you allow destructive scenarios (and never while signed in).
...
Run Hound can sign in as one of two test accounts you own on your app, A and B, before it tests. Every check then runs signed in, so pages behind your sign-in can be tested, and six checks join the plan: `access-control`, `mass-assignment` and `deep-links` (0.4.0), and the write-side checks `write-access`, `csrf` and `paywall-trust` (0.5.0, unticked by default). They are a preview: tell us what they got right and wrong.

### Try it on Fernway first

Fernway is the test lab's SaaS app. It runs twice: `fernway` (clean) and `fernway-bugs` (with every planted bug, including the access bugs V01-V05 and the write-side bugs V06-V09). Its two accounts are Alex (`alex@fernway.test`, account A) and Sam (`sam@fernway.test`, account B), each with a workspace of their own; the passwords are in [fixtures/fernway/README.md](fixtures/fernway/README.md#accounts). To look around first, open <http://localhost:4111/login> and press **Use the demo account**.
...
8. The write-side checks: plan `http://fernway-bugs:4110/app` again, tick `write-access` (unticked by default) and run it. Expect account B changing Alex's test task (V06) and a visitor who isn't signed in changing it (V07); the notes say the task was put back. Plan `http://fernway-bugs:4110/app/settings`, tick `paywall-trust` and run it: opening the upgrade success page gives Alex Pro without paying (V09), and the notes say the plan was put back. `csrf` is **inconclusive** in the test lab, because `fernway-bugs` is a container name with no cross-site twin; to see it catch V08, run Fernway from source and test `http://localhost:4111/app` (below).
...
### The write-side checks (0.5.0)

Three checks test **writes**. They change account A's data on purpose, so all three are **unticked by default**, and all three follow the same rules:

- They write **only the test record Run Hound created as A in the same scenario** (it carries the run's token). Never one of A's own records, never an id found by listing, guessing or counting up. A page with no form that creates such a record gets the scenario skipped with the reason.
...
- A write counts as working only when **a re-read as A** shows the change (the value changed, or the record is gone). The status code never decides: a `200 {"error": …}` or an ignored write is common.
- Afterwards they **put it back** (the original values, or the record again when it was deleted; A's plan for `paywall-trust`), re-read it and compare. Whatever couldn't be undone is named in the notes ("… check Account A"), and the scenario isn't a pass while that note stands. A stop or the time limit can end a scenario between a write and its restore: the report then asks you to check account A.

What each one does:

- **`write-access`**. As A, it creates the test record through the form, then finds the update and delete requests **the app itself sent** for that record (a `PUT`, `PATCH` or `POST` to a URL with the record's id; a `DELETE` only when the app showed one, and last). It sends each as account B (scenario `other-account`, needs B and "must not see each other's data") and with no session (`signed-out`), with one field changed to a new test value, and re-reads the record as A after each. A change or a deletion is a critical finding.
- **`csrf`**. As A, it creates the test record, then opens a page on **another site** in A's own browser (`127.0.0.1` when your app is on `localhost`, and the other way round; another port is the same site and doesn't count) and sends the form's save from there with a new value. Only requests any web page can send without a CORS preflight are forged (form-encoded or `text/plain` bodies, no custom headers, nothing added), and the browser attaches only the cookies it would for any website (a `SameSite=Lax` cookie stays home). A forged value that shows when A re-reads the record is a high finding; the notes say which defence was missing. On any other host name (`host.docker.internal`, a name from `RUNHOUND_ALLOWED_HOSTS`, a container name in the test lab) there is no cross-site address to use, and the scenario is **inconclusive**: never a pass, never a finding. A save sent only as JSON that the app doesn't also accept form-encoded or as text passes with "needs a preflight".
- **`paywall-trust`**. It finds A's plan or entitlement data (`plan`, `tier`, `subscription`, `isPro`, `credits`, `entitlements`, `features`) the way `mass-assignment` finds a record, and is skipped when there is none or when A already has a paid plan. It then opens your app's own success/upgraded/thank-you pages that the page links to, replays your app's own upgrade request (to your app only) with a zero price or a paid plan, and calls an API the app uses only in paid mode, re-reading A's plan after each. A paid plan without paying is a critical finding; a paid API answering with data is high; text such as "Pro" in the page is at most advisory. It **never enters payment details and never loads or calls a payment provider**: requests to one are blocked and listed in the notes.
