 ...
 * FERNWAY_BUGS, call its API, and drive it with Playwright while collecting console errors, page errors, failed
 * requests and 4xx/5xx responses. globalSetup (test/global-setup.ts) builds dist/ once before any test file.
 */
...
export const ALL_BUGS = ["W01", "W02", "W03", "W04", "W05", "W06", "W07", "W08", "W09", "W10"] as const;
...
/** The seeded account (server/seed.mjs). */
export const DEMO_ACCOUNT = { email: "alex@fernway.test", password: "correct-horse-battery" } as const;
...
export interface Fernway {
  ...
  output(): string;
  /** POST /api/__reset (restores the seed and clears the idempotency cache). */
  reset(): Promise<void>;
  stop(): Promise<void>;
}
 ...
 * (sent as is, for malformed-JSON tests).
 */
