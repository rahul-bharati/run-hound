...
- **Fernway** (`fixtures/fernway`), a second test app built the way Lovable, Bolt and v0 build apps (Vite, React 19, Tailwind CSS 4, Radix/shadcn-style components, react-hook-form with zod, sonner, cmdk, React Router): a landing page, sign-up, sign-in, an onboarding wizard, and a dashboard and settings behind a real sign-in with two accounts, in light and dark. Clean mode is a false-positive suite for modern UIs; `FERNWAY_BUGS` plants W01-W10 (bugs AI-built apps typically ship with) and V01-V05 (access bugs for the V2 checks). Both compose files run it as `fernway` (clean, host port 4110) and `fernway-bugs` (host port 4111), from a fourth image, `ghcr.io/rahul-bharati/run-hound-fernway`. It has its own tests, and the acceptance suite runs Run Hound against it, signed in for `/app` and `/app/settings`.
...
- **Releases run CI first**: the release workflow checks the tag against `app/package.json`, both compose files and the Dockerfile comments, then runs the whole CI workflow, and only then builds and pushes the four images (linux/amd64 and arm64; tags `0.4.0`, `0.4` and `latest`) and starts each arm64 image as a check. CI also builds the four images, checks that Run Hound's stays Chromium-only and under 1 GB, starts the compose test lab and smoke-tests it (every app answers, a CLI run on Kennel exits 1 with its findings), and lints and builds the website.
