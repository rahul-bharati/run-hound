...
- **Test accounts and signed-in runs**: two accounts you own on your app (A and B) in Settings → Test accounts or `run-hound accounts set|status|test|clear`; `run --as a|b` and New Run → **Sign in as** sign in first and run every check signed in, with a fresh session for each plan and run. Plans, reports and the runs list say which account ran ("Signed in as Account A"). Passwords are write-only and bound to their sign-in site; passwords, session cookies and tokens are redacted from reports, evidence, specs, logs, progress and AI prompts, and accounts are named by label.
- **Access checks** (`access-control`): can account B, or a signed-out visitor, read account A's data? Read-only: only the GET requests that returned A's data are replayed. **Mass assignment** (`mass-assignment`, unticked by default): does the server store `role`, `plan` or `isAdmin` fields the form never sends? It restores what it changed and says what it couldn't. **Deep links** (`deep-links`, also planned signed out): do the app's own pages load when opened directly? Signed out, the plan says to sign in as a test account to run the access checks.
- **CLI, API and settings for the accounts**: `run-hound accounts status | test [a|b] | set a|b [--login-url] [--username] [--label] [--password-stdin] | clear a|b` (the password only ever from stdin) and `run --as a|b`; the Docker entrypoint takes `accounts`. `GET/PUT /api/accounts`, `POST /api/accounts/test`, and `POST /api/plan` takes `signInAs`. `accounts.json` (mode 0600) sits next to `ai.json`; `RUNHOUND_ACCOUNT_{A,B}_{LOGIN_URL,USERNAME,PASSWORD,LABEL}` and `RUNHOUND_ACCOUNTS_ISOLATED` override it, and both compose files pass them through (empty by default, documented in `.env.example`).
...
- **Schema-validated forms** (react-hook-form with zod, which mark nothing as required): every field the checks fill is filled, optional ones included, and the fields a form refuses when it is sent empty count as required, so `error-announcement` is planned for them. The empty submit is answered by Run Hound, so nothing is saved.
- **Multi-step forms**: when submitting shows the next step without saving, the checks that need a saved record skip with a plain "multi-step form" note instead of blaming the test values. Only the first step is tested.
...
- **Fernway** (`fixtures/fernway`), a second test app built the way Lovable, Bolt and v0 build apps (Vite, React 19, Tailwind CSS 4, Radix/shadcn-style components, react-hook-form with zod, sonner, cmdk, React Router): a landing page, sign-up, sign-in, an onboarding wizard, and a dashboard, settings and a help page behind a real sign-in with two accounts, in light and dark. Clean mode is a false-positive suite for modern UIs; `FERNWAY_BUGS` plants W01-W10 (bugs AI-built apps typically ship with) and V01-V05 (access bugs for the V2 checks). Both compose files run it as `fernway` (clean, host port 4110) and `fernway-bugs` (host port 4111), from a fourth image, `ghcr.io/rahul-bharati/run-hound-fernway`. It has its own tests, and the acceptance suite runs Run Hound against it, signed in for `/app`, `/app/settings` and `/app/help`. With every bug on, each page but `/app/help` (V05) loads directly, so the lab's `fernway-bugs` service can be planned on `/app/settings` as it is.
- **Health checks** for every service in both compose files, so `docker compose ps` shows which are `healthy`. The Run Hound image has its own `HEALTHCHECK`: healthy while `serve` answers, and a one-shot `run` counts as healthy.

### Changed

- **A much smaller Run Hound image**: `node:24.21.0-bookworm-slim` with Chromium's headless shell only (plus fonts for the evidence cards), 715 MB on disk and about 260 MB to download, down from 2.77 GB on Playwright's image with every browser. The four lab images together download about 0.5 GB. A container has no display, so `--headed` stays refused there; watch the browser from a source install on a desktop. Run Hound's, Kennel's and the samples' images are built on an exact Node release.
- **Releases run CI first**: the release workflow checks the tag against `app/package.json`, the website's version, both compose files, the Dockerfile comments and every tag-pinned download link in the compose files, README and TESTING, then runs the whole CI workflow, and only then builds and pushes the four images (linux/amd64 and arm64; tags `0.4.0`, `0.4` and `latest`) and starts each arm64 image as a check. CI also builds the four images, checks that Run Hound's stays Chromium-only and under 1 GB, starts the compose test lab and smoke-tests it (every app answers, a CLI run on Kennel exits 1 with its findings), and lints and builds the website.
- **Kennel has a new look**: a warm, modern design with an illustrated hero, pet-type tiles and booking cards. Its contract, planted bugs and golden files are unchanged.
- The web UI and the HTML report say "V2 preview" (0.3.0: "V1 preview").
- A page that sends a signed-out visit to a sign-in page now suggests signing in as a test account, instead of saying that pages behind a login aren't supported.
...
- On Kennel, A03 is now also reported by `error-announcement`.
...
- `keyboard-completion` sets widgets with the keyboard (Space, arrows, Enter, typing) and no longer reports a form it typed nothing into. `axe-states` scans and lists only the form states it reached (saying why it didn't reach the others) and closes a list left open before scanning. Skip notes name the fields Run Hound couldn't set.
...
- `dead-control` and `page-controls` report a button an invisible element covers (a leftover backdrop) as one nobody can click, and a scenario in which no control could be clicked is skipped instead of passing. They leave out sending, paying, ordering and trash-icon controls unless `--allow-destructive`; a contact form's own "Send message" button still works with Enter in a suggested flow.
...
- Docker: on a rootful engine with a runs folder Docker created as root, the folder is handed to the image's user (uid 1001) with normal permissions (755/644) instead of being made world-writable; deleting reports then needs `sudo`, and the entrypoint says so. The saved settings folder (`runs/.config`, with `ai.json` and `accounts.json`) is given to the same user.
...
- **Docker images on GHCR**: a release workflow publishes `ghcr.io/rahul-bharati/run-hound`, `run-hound-kennel` and `run-hound-samples` (linux/amd64 and linux/arm64) when a version tag is pushed, with an optional Docker Hub mirror. The docs and the compose files use the GHCR names; `docker compose up --build` in a clone builds the same images.
- **No-clone test lab**: `run-hound.compose.yml` starts Run Hound, Kennel (broken and clean) and the five sample apps from the published images, with nothing to clone or build: download it, `mkdir -p runs` and `docker compose -f run-hound.compose.yml up`. The image's entrypoint also takes `ai`.
...
## Between 0.1.0 and 0.2.0 (shipped in 0.2.0)
