...
import { AI_CHECK_IDS, CHECK_IDS, V2_CHECK_IDS } from "../../../app/src/core/types.js";
...
describe.concurrent("Run Hound against Kennel", () => {
  it.for(modes)("mode %s matches its accepted response", async (mode, { expect }) => {
    ...
    try {
      ...
      // AI-only checks (ai-flow) plan nothing without a model. The V2 checks plan nothing on Kennel either
      // (docs/v2-spec.md "Checks"): access-control and mass-assignment need a signed-in run, and deep-links a page
