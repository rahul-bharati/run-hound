    ...
    <div className={`flex max-w-full flex-col items-start gap-2 ${className}`}>
      ...
      <div className="flex w-full items-center gap-3 rounded-xl border border-line-strong bg-surface py-2 pl-4 pr-2 font-mono text-sm">
        ...
        <button
          ...
          onClick={copy}
          disabled={comingSoon}
          className="min-h-11 shrink-0 rounded-lg border border-line px-3 text-xs font-medium text-muted enabled:hover:border-amber enabled:hover:text-amber disabled:cursor-not-allowed disabled:text-dim"
        >
          {copied ? "Copied" : "Copy"}
        </button>
