...
- **Page checks** plan once, with `scope: "page"` and `scopeLabel: "Whole page"`. Page-scoped: `bundle-secrets`, `reflow-320` (one set of scripts and one layout per page) and the five V1 checks.
...
- The runner gives each scenario its own form (`CheckContext.form`) and the whole page (`CheckContext.discoveredPage`).
