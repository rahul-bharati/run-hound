...
import { guardContext } from "./guard.js";
...
beforeAll(async () => {
  browser = await chromium.launch({ args: ["--host-resolver-rules=MAP blocked.test 127.0.0.1"] });
...
async function guarded() {
  ...
  const guard = await guardContext(context, { lookup });
